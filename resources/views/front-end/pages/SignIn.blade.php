...
@endif 
    <form class="form-signin"  action="{{ route('log-in-check') }}"  method="post" enctype="multipart/form-data" autocomplete="off" >
       @csrf
     ...
     <div>
        <input type="email" id="inputEmail" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="Email address" required autocomplete="current-email" >
        @error('email')
